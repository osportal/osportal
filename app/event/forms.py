...
class EventForm(FlaskForm):
    start_date = DatePickerField('Start Date', validators=[check_leave_year_start,
                                                           DataRequired()])
    end_date = DatePickerField('End Date', validators=[check_end_date, DataRequired()])
    etype = QuerySelectField('Event Type', query_factory = lambda: EventType.query.filter(EventType.active==True).all(), get_pk=lambda et: et.id, widget=Select2Widget(), allow_blank=False, validators=[DataRequired(), check_approval], render_kw={"id": "event-type-select"})
    details = TextAreaField('Details', validators=[Optional(), Length(min=2, max=1000)], render_kw={"placeholder": "(Optional)"})
